...
def no_moves_against_iteration(df):
    ...
    df.plot(ax = axes[1,1], y = "elapsed_time_in_milliseconds",title = f'Elapsed Time, avg:{df["elapsed_time_in_milliseconds"].mean()}')
    
    

    local_df = df.sort_values(by=['no_moves'],ascending=False)
    local_df.plot(ax = axes[2,0], x ="no_moves", y = "elapsed_time_in_milliseconds",title = 'Elapsed Time')
...
if __name__ == "__main__":
    ...
    df = pd.DataFrame(columns=["iteration","color","no_moves","position_estimation","best_move_estimation","moved_piece","elapsed_time_in_milliseconds",])
    for id, x in enumerate(whole_file_buffer):
        ...
        try:
            ...
            print(x)
            data_point = pd.DataFrame({'iteration':int(x[1]), 'color':x[3], 'no_moves':int(x[5]), 'position_estimation':int(x[7]),'best_move_estimation':float(x[9]),'moved_piece':x[11],'elapsed_time_in_milliseconds':int(x[13])}, index=[0])
