...
class SimulateGameTestCase:
    ...
    def parse_string(test_str: str):
        ...
        if no_moves > 0:
            starting_positions = [
                int(pos) for pos in param_dict["starting_positions_str"].split(" ")
            ]
            moves = [int(pos) for pos in param_dict["moves_str"].split(" ")]
            moves_estimation = [
                int(pos) for pos in param_dict["moves_estimation"].split(" ")
            ]
...
if __name__ == "__main__":
    ...
    buffer = ""
    chess_engine_proc = subprocess.Popen(
        [
            r"C:\Users\piotr\Documents\chess-mk3\chess_board\buildx64\Debug\ChessEngineMk3.exe"
        ],
        ...
        for line in chess_engine_proc.stdout:
            ...
            try:
                if "iteration_end" in str(resp):
                    ...
                    test_example.board.display_using_unicode_minimalistic(
                        use_class_variables=True
                    )
